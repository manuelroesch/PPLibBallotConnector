...
class BallotPortalAdapterTest {
  ...
  val daoTest = new DAOTest()
	...
	def testProcessQuery: Unit = {
		...
		val query = HTMLQuery(<div>
			<h1>test</h1> <form>
        <input type="submit" name="answer" value="yes"> </input>
      </form>
		</div>)
		val prop = new BallotProperties(Batch(), List(Asset(Array.empty[Byte], "application/pdf", "empty filename")), 1, 10)
    ...
    Assert.assertEquals(ans.asInstanceOf[Option[HTMLQueryAnswer]].get.answers.get("answer").get, "yes")
		...
		Assert.assertEquals(daoTest.outputCode.size, 1)
	}

	@Test
	def testWithoutForm: Unit = {
		val b = new BallotPortalAdapter(new PortalAdapterTest(), new DAOTest(), "http://www.andreas.ifi.uzh.ch:9000/")
		val ans = b.processQuery(HTMLQuery(<h1>test</h1>), new BallotProperties(Batch(), List(Asset(Array.empty[Byte], "application/pdf", "empty filename")), 1, 10))
	...
	def testDeepFormStructure: Unit = {
		...
		</div>), new BallotProperties(Batch(), List(Asset(Array.empty[Byte], "application/pdf", "empty filename")), 1, 10))
		Assert.assertEquals(ans.asInstanceOf[Option[HTMLQueryAnswer]].get.answers.get("answer").get, "yes")
	}
	...
	def testWithInvalidInput: Unit = {
		...
		val ans = b.processQuery(HTMLQuery(<div>
			<h1>test</h1> <div>
				<form>
					<p>
						<input type="button" name="answer" value="yes"> </input>
					</p>
				</form>
			</div>
		</div>), new BallotProperties(Batch(), List(Asset(Array.empty[Byte], "application/pdf", "empty filename")), 1, 10))
  ...
  @Test
  def testWithoutFormActionAndMethod: Unit = {
    val b = new BallotPortalAdapter(new PortalAdapterTest(), new DAOTest(), "http://www.andreas.ifi.uzh.ch:9000/")
    val ans = b.processQuery(HTMLQuery(<div>
      <h1>test</h1> <form><input type="submit" value="yes" name="answer" > </input></form>
...
class DAOTest extends DAO with LazyLogger {
  ...
  val assets = new mutable.HashMap[Long, Long]
	...
	val answers = new mutable.HashMap[Long, (String, Boolean)]
  val outputCode = new mutable.HashMap[Long, String]()
	...
	override def getAnswers(questionId: Long): List[String] = {
		answers.get(questionId).map(a => a._1).toList
	}
	...
	override def createQuestion(html: String, batchId: Long, uuid: UUID = UUID.randomUUID(), dateTime: DateTime = new DateTime()): Long = {
		...
		answers += ((answers.size + 1).toLong -> ("{\"answer\":\"yes\"}", false))
    outputCode += ((answers.size+1).toLong -> Math.abs(Random.nextLong()).toString)
  ...
  override def updateAnswer(answerId: Long, accepted: Boolean): Unit = {
    if(answers.get(answerId).isDefined){
      answers.update(answerId, (answers.get(answerId).get._1, accepted))
    }
  }

  override def getAnswerIdByOutputCode(insertOutput: String): Option[Long] = {
    Some(outputCode.filter(o => o._2.equals(insertOutput)).headOption.getOrElse((-1L, None))._1)
  }

  override def getExpectedOutputCodeFromAnswerId(ansId: Long): Option[Long] = {
    val expected = outputCode.get(ansId).getOrElse("").toLong
    Some(expected)
  }
}
