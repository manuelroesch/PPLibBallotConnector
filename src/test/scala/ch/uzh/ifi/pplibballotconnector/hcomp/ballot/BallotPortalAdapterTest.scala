...
class BallotPortalAdapterTest {
  ...
  def testProcessQuery: Unit = {
    ...
    val query = HTMLQuery(<div><h1>test</h1><form action="http://www.andreas.ifi.uzh.ch:9000/storeAnswer" method="post"><input type="submit" name="answer" value="yes" /></form></div>)
  ...
  def testWithoutBallotProperties: Unit = {
    ...
    val ans = b.processQuery(HTMLQuery(<div><h1>test</h1><form action="http://www.andreas.ifi.uzh.ch:9000/storeAnswer" method="post"><input type="submit" name="answer" value="yes" /></form></div>),
  ...
  def testDeepFormStructure: Unit = {
    val b = new BallotPortalAdapter(new PortalAdapterTest(), new DAOTest(), "http://www.andreas.ifi.uzh.ch:9000/")
    val ans = b.processQuery(HTMLQuery(<div><h1>test</h1><div><form action="http://www.andreas.ifi.uzh.ch:9000/storeAnswer" method="post"><p><input type="submit" name="answer" value="yes"/></p></form></div></div>), new BallotProperties(new Batch(), 1, 123))
    Assert.assertEquals(ans.asInstanceOf[Option[HTMLQueryAnswer]].get.answers.get("answer").get, "yes")
  }
  ...
  def testWithoutFormsInput: Unit = {
    val b = new BallotPortalAdapter(new PortalAdapterTest(), new DAOTest(), "http://www.andreas.ifi.uzh.ch:9000/")
    val ans = b.processQuery(HTMLQuery(<div><h1>test</h1><form action="http://www.andreas.ifi.uzh.ch:9000/storeAnswer" method="post"></form></div>), new BallotProperties(new Batch(), 1, 123))
    Assert.assertEquals(ans, None)
  }
